...
function parseReasons(value) {
  ...
  if (typeof value === 'string') {
    ...
    if (!Array.isArray(parsed) || parsed.length === 0) {
      throw new Error('Conjunction returned an invalid or empty reasons payload.');
    }
...
function requireFiniteNumber(value, fieldName) {
  ...
  if (!Number.isFinite(number)) {
    throw new Error(`Conjunction response is missing or has an invalid ${fieldName}.`);
  }
...
function formatUTC(timestamp, fieldName) {
  ...
  if (Number.isNaN(date.getTime())) {
    throw new Error(`Conjunction response contains an invalid ${fieldName}.`);
  }
...
function formatTCA(timestamp) {
  ...
  if (Number.isNaN(date.getTime())) {
    throw new Error('Conjunction response contains an invalid tca_timestamp.');
  }
...
export default function EventDetail({ event }) {
  if (!event) {
    return <FieldError message="No conjunction event was supplied to EventDetail." />;
  }
  ...
  try {
    ...
    const nameA = getObjectName(event.object_a, 'object_a');
    const nameB = getObjectName(event.object_b, 'object_b');
    ...
    const riskScore = requireFiniteNumber(event.risk_score, 'risk_score');
    const confidence = requireFiniteNumber(event.confidence, 'confidence');
    const reasons = parseReasons(event.reasons);

    const objectADetails = getObjectDetails(event.object_a);
    const objectBDetails = getObjectDetails(event.object_b);
    ...
    data = {
      ...
      objectADetails,
      objectBDetails,
      epoch: event.epoch || event.epoch_utc || null,
  ...
  const epochDisplay = epoch ? formatUTC(epoch, 'epoch') : null;
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 p-4 rounded-lg bg-slate-900/80 border border-slate-800">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="text-right px-4 py-2 rounded-lg bg-slate-950/80 border border-slate-800">
              ...
              <span className="text-lg font-mono font-extrabold text-white">
                Confidence:{' '}
                <span className="text-emerald-400">{confidence}/100</span>
              </span>
            </div>
          </div>
        </div>
      ...
      <section className="glass-panel rounded-xl p-5 border border-slate-800">
        ...
        {/* Object specifications — only render fields actually supplied */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="p-4 rounded-lg bg-slate-900/60 border border-cyan-500/30">
            ...
            <div className="space-y-2 text-xs font-mono">
              <div className="flex justify-between py-1 border-b border-slate-800/50">
                ...
                <span className="text-white font-semibold">{nameA}</span>
              </div>
              ...
              {objectADetails?.type && (
                <div className="flex justify-between py-1 border-b border-slate-800/50">
                  ...
                  <span className="text-slate-200">{objectADetails.type}</span>
                </div>
              )}
          ...
          <div className="p-4 rounded-lg bg-slate-900/60 border border-rose-500/30">
            ...
            <div className="space-y-2 text-xs font-mono">
              <div className="flex justify-between py-1 border-b border-slate-800/50">
                <span className="text-slate-400">Common Name:</span>
                <span className="text-white font-semibold">{nameB}</span>
              </div>
              ...
              {objectBDetails?.type && (
                <div className="flex justify-between py-1 border-b border-slate-800/50">
                  <span className="text-slate-400">Object Classification:</span>
                  <span className="text-slate-200">{objectBDetails.type}</span>
                </div>
              )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="p-4 rounded-lg bg-slate-900/60 border border-slate-800">
            ...
            <p className="text-xs text-slate-300 leading-relaxed">
              ...
              <span className="font-bold text-white">{tca}</span>. Any
              operational decision must therefore be evaluated against the
              actual TCA supplied by the conjunction calculation.
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="space-y-3">
          ...
          <div className="p-3 rounded-lg bg-slate-950/60 border border-slate-800/80 text-xs font-mono text-slate-400 flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              {epochDisplay ? (
                <span className="text-slate-300">{epochDisplay}</span>
              ) : (
